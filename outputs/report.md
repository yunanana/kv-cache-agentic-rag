...
## SUMMARY
- **평가 범위** : KV cache 메모리 병목에 대한 SW 접근 TurboQuant(Google, KV cache 약 3비트 양자화)과 HW 접근 ITME(SK hynix, CXL-하이브리드 메모리 기반 계층 확장)를 데이터센터·클라우드 LLM 서빙 기준으로 시장성·도메인 적용 두 관점에서 평가했다. 우열이나 채택 권고가 아니라 관점별 평가 차이를 정리한 것이다.
- **품질** : TurboQuant은 저자 보고 기준 Llama-3.1-8B-Instruct의 Needle-In-A-Haystack 실험에서 KV cache를 25%만 쓰는 4배 압축 조건에서도 원본과 같은 리콜 0.997을 기록했다[1, p.17, p.19]. 해당 실험의 점수 유지이며 무손실 압축의 입증은 아니다. ITME는 원본 KV를 그대로 보관하는 구조이나 정확도 측정 수치는 보고하지 않았다[2, p.2].
- **성능·비용** : ITME는 저자 보고 기준 CPU 오프로딩 대비 최대 35.7% 처리량 향상을 보였으나[2, p.1, p.11], CXL-하이브리드 메모리·RDMA·FPGA 등 신규 하드웨어가 필요하고 비용 수치는 없다[2, p.2, p.8]. TurboQuant의 메모리 절감은 비용 절감의 간접 근거이며, 웹 자료가 인용한 vLLM 연구는 FP8·BF16 대비 처리량 20~34% 저하와 최악 구성의 지연 60~68% 증가도 보고한다[5][8].
- **시장·채택** : 두 기술 모두 공식 상용 제품이나 서빙 스택 기본 탑재 근거는 없다. TurboQuant은 vLLM·SGLang 기반 구현 보고가 있으나 Google 실서비스 적용은 공개 근거가 없고[5], ITME는 프로토타입 단계다[2, p.1]. 시장성 등급은 TurboQuant 중간(인접 시장 근거), ITME 높음(CXL 시장 추정치)이다[3][4].
...
- **불확실성** : 핵심 수치 대부분이 저자 자체 실험이고 웹 출처는 2차 자료다. ITME의 정확도·TTFT, TurboQuant의 대형 모델·다중 GPU 서빙 성능, 두 기술의 실제 TCO는 확인되지 않았다(5.3 참고).
...
평가 도메인은 **데이터센터·클라우드 LLM 서빙**으로 한정했다. 다수 사용자의 요청을 GPU 클러스터에서 처리하는 환경으로, 처리량·지연 목표(SLO)와 총소유비용(TCO)에 민감하고, 긴 문맥과 동시 사용자 증가로 KV cache가 GPU 메모리 병목이 되는 대표적인 환경이다. 온디바이스 등 다른 도메인에서는 같은 기술도 평가가 달라질 수 있다.

## 2. 기술 선정
과제 Doc Pool에서 진영별 1건을 직접 선정했다. 기준은 (1) 해당 진영의 접근을 대표하는지, (2) 시장성·도메인 평가에 필요한 공개 근거를 확보할 수 있는지, (3) 두 기술이 서로 대비되는지다.
...
**평가 방법** : 기술별 논문 원문을 검색(RAG)해 저자 보고 수치와 실험 조건을 확인하고, 웹 검색으로 시장·채택·실사용 반응을 보강했다. 기준마다 지지 근거와 반대 근거를 함께 수집했고, 논문 수치는 '저자 보고 기준', 대상 기술이 아닌 인접 기술·시장 자료는 '간접 근거'로 구분했다.

**등급과 신뢰도** : 등급은 해당 관점에서 기술이 유리하게 인식되는 정도이며 두 기술 간 우열이 아니다. **높음**은 기준 조건을 충족하는 근거가 명확한 경우, **중간**은 조건부이거나 근거가 간접적인 경우, **낮음**은 불리한 근거가 우세한 경우, **판단 유보**는 근거가 없는 경우다. **신뢰도**는 근거의 양과 독립성(저자 자체 실험만 있는지, 제3자 자료가 있는지)을 나타낸다.

| 관점 | ID | 기준 | 평가 대상 |
|---|---|---|---|
| 시장성 | M1 | 시장 규모·성장성 | 기술이 속한 시장의 규모와 성장 전망 |
| 시장성 | M2 | 상용화·채택 현황 | 제품 출시, 서비스 도입, 오픈소스 통합 사례 |
| 시장성 | M3 | 생태계 지지 | 지원 프레임워크, 표준화, 벤더 참여 |
| 도메인 | D1 | 비용 효율 (TCO) | 같은 워크로드에 필요한 GPU·메모리 비용의 절감 여지 |
| 도메인 | D2 | 처리량·지연 | 대규모 동시 요청 환경의 처리량·지연·SLO 영향 |
| 도메인 | D3 | 품질 유지 | 정확도 손실 위험 (정확도 지표만 근거로 인정, 지연·처리량 수치는 제외) |
...
### 후속 검증이 필요한 사항
이 보고서의 근거로는 확인되지 않아, 도입 판단 전에 별도 검증이 필요한 항목이다.
- **ITME** : 정확도·TTFT 등 서비스 품질 지표 (논문은 처리량 결과 중심이며 정확도 측정은 없음)[2, p.1, p.11]
- **ITME** : FPGA 프로토타입이 아닌 양산형 CXL 장치에서의 성능과 비용
- **TurboQuant** : 8B보다 큰 모델, 다중 GPU·대규모 동시 요청 서빙 환경에서의 처리량·지연 실측 (논문 실험은 단일 A100)[1, p.15]
- **공통** : 장비·전력·운영비를 포함한 실제 TCO, 그리고 압축한 KV cache를 CXL 계층에 저장하는 병행 구성의 효과
