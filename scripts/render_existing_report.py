...
def main():
    ...
    # 수동 렌더링 대상은 자동 검토 통과본이 아니므로, 검토 상태(미통과 표시 또는 사람 검수 명시)가 본문에 있어야 한다
    if '검토 상태:' not in md and '사람 검수본' not in md:
        raise ValueError('수동 렌더링에는 검토 상태 또는 사람 검수 사실을 먼저 명시해야 합니다')
